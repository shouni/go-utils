...
| **`urlpath`** | **URLやリモートURI（GCS/S3）の解析・解決**を行い、クラウドとローカルパスを透過的に扱います。 | クラウドURI判定 (`IsRemoteURI`)、出力パス解決 (`ResolveOutputPath`)、ベースURL解決 (`ResolveBaseURL`)、**Webサービスのセキュア判定** (`IsSecureServiceURL`) | **機能拡充** |
